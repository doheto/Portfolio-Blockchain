...
contract("LeveragedYieldFarm", ([acc]) => {
  ...
  beforeEach(async () => {
    ...
    daiSwapAddress = await uniswap.methods
      .getExchange(legos.erc20.dai.address)
      .call();
  ...
  describe("Uniswap", () => {
    it("swapping 1 ETH for DAI...", async () => {
      ...
      await daiSwap.methods
        .ethToTokenSwapInput(1, 2525644800)
        .send({ from: acc, value: web3.utils.toWei("1", "Ether") });
  ...
  describe("Leveraged Yield Farming on Compound boosted with dYdX flash loan...", () => {
    beforeEach(async () => {
      ...
      await dai.methods
        .transfer(contract.address, web3.utils.toWei("1.1", "Ether"))
        .send({ from: acc });
    ...
    it("deposit/wait/withdrawing/taking profits...", async () => {
      const ethBalanceBefore = await web3.eth.getBalance(acc);
      const daiBalanceBefore = web3.utils.fromWei(
        await dai.methods.balanceOf(acc).call()
      );
      const cDaiBalanceBefore = web3.utils.fromWei(
        await cDai.methods.balanceOf(contract.address).call()
      );
      const compBalanceBefore = web3.utils.fromWei(
        await comp.methods.balanceOf(acc).call()
      );
      ...
      const ethBalanceAfter = await web3.eth.getBalance(acc);
      const daiBalanceAfter = web3.utils.fromWei(
        await dai.methods.balanceOf(acc).call()
      );
      const cDaiBalanceAfter = web3.utils.fromWei(
        await cDai.methods.balanceOf(contract.address).call()
      );
      const compBalanceAfter = web3.utils.fromWei(
        await comp.methods.balanceOf(acc).call()
      );
