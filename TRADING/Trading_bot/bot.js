...
const main = async () => {
  const {
    ...
    token0,
    token1
  } = await getTokenAndContract(arbFor, arbAgainst);
...
const determineProfitability = async (
  ...
  console.log(
    `SHIB: ${Number(
      web3.utils.fromWei(reserves[0].toString(), "ether")
    ).toFixed(0)}`
  );
  ...
  try {
    ...
    let result = await _routerPath[0].methods
      .getAmountsIn(reserves[0], [_token0.address, _token1.address])
      .call();

    const token0In = result[0]; // WETH
    const token1In = result[1]; // SHIB

    result = await _routerPath[1].methods
      .getAmountsOut(token1In, [_token1.address, _token0.address])
      .call();

    console.log(
      `Estimated amount of WETH needed to buy enough Shib on ${exchangeToBuy}\t\t| ${web3.utils.fromWei(
        token0In,
        "ether"
      )}`
    );
    ...
    let wethBalanceBefore = await _token0Contract.methods
      .balanceOf(account)
      .call();
...
const executeTrade = async (_routerPath, _token0Contract, _token1Contract) => {
  ...
  const data = {
    ...
    "ETH Spent (gas)": web3.utils.fromWei(
      (ethBalanceBefore - ethBalanceAfter).toString(),
      "ether"
    ),
    "-": {},
    "WETH Balance BEFORE": web3.utils.fromWei(
      balanceBefore.toString(),
      "ether"
    ),
    "WETH Balance AFTER": web3.utils.fromWei(balanceAfter.toString(), "ether"),
    "WETH Gained/Lost": web3.utils.fromWei(
      balanceDifference.toString(),
      "ether"
    ),
    "-": {},
    "Total Gained/Lost": `${web3.utils.fromWei(
