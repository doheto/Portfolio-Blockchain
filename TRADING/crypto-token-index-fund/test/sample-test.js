...
describe("Token contract", function() {
  ...
  beforeEach(async () => {
    ...
    console.log("Contract deployed", indexFundContract.address);
  });
  ...
  it("should return 300 ether balance of contract", async function() {
    ...
    const user1Address = "0xBE0eB53F46cd790Cd13851d5EFf43D12404d33E8"; //check etherscan then top
  ...
  it("should get the price of all defi tokens", async function() {
    ...
    console.log(
      "compount token price in ether = ",
      compPrice.toString() / 1e18
    );

    const snxPrice = await indexFundContract.snxPrice();
    console.log(
      "synthetix token price in ether = ",
      snxPrice.toString() / 1e18
    );
    ...
    console.log(
      "kyber network crystal token price = ",
      kncPrice.toString() / 1e18
    );
  ...
  it("should buy 3 tokens", async function() {
    // unlock account
    const user2Address = "0xDA9dfA130Df4dE4673b89022EE50ff26f6EA73Cf";
    await hre.network.provider.request({
      method: "hardhat_impersonateAccount",
      params: [user2Address]
    });
    ...
    console.log(
      "before buying ether balance of user2 = ",
      balanceEther.toString() / 1e18
    );
    console.log(
      "before buying token balance of user2 = ",
      balanceTokens.toString() / 1e18
    );

    await indexFundContract
      .connect(user2)
    ...
    console.log(
      "after buying ether balance of user2 = ",
      balanceEther.toString() / 1e18
    );
    console.log(
      "after buying tokens balance of user2 = ",
      balanceTokens.toString()
    );
  ...
  it("should test the defi increased and redeemToken function", async function() {
    ...
    balanceEther = await provider.getBalance(user2Address);
    console.log(
      "after redeeming ether balance of user2 = ",
      balanceEther.toString() / 1e18
    );
    console.log(
      "after redeeming tokens balance of user2 = ",
      balanceTokens.toString()
    );
  });
});
