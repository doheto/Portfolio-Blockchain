...
// to interact with coumpound token
interface cERC20 {
    function mint(uint256) external returns (uint256);//when u provide USDC u get CUSDC
    function balanceOf(address owner) external view returns (uint256 balance);
    function redeem(uint256) external returns (uint256);//this helps us to get our original usdc back
    function exchangeRateCurrent() external returns (uint256);
}

contract Escrow is Ownable {
    address public payee;//the entrepreneur
    uint256 public currentStage = 1;
    uint256 public totalStages;// number of milestones
    mapping(uint256 => uint256) holderRewardsPerStage;//from compound
